...
static int module_foreach(int argc, const char **argv, const char *prefix)
{
	...
	for_each_listed_submodule(&list, runcommand_in_submodule, &info);
				      ...
				      struct module_cb_list *list)
{
	...
	for (i = 0; i < list->nr; i++) {
		...
		if (info->for_status) {
			...
			const char *value;
			const struct submodule *sub = submodule_from_path(&null_oid, p->sm_path);
...
static int module_summary(int argc, const char **argv, const char *prefix)
{
	...
	if (!capture_command(&cp_rev, &sb, 0)) {
		...
		struct stat st;
		struct object_id oid;
		if (fstat(0, &st) < 0 || index_fd(&oid, 0, &st, 2, prefix, 3))
...
static int module_sync(int argc, const char **argv, const char *prefix)
{
	...
	for_each_listed_submodule(&list, sync_submodule, &info);
...
static int module_deinit(int argc, const char **argv, const char *prefix)
{
	...
	for_each_listed_submodule(&list, deinit_submodule, &info);
