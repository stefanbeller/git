...
static int fall_back_threeway(const struct am_state *state, const char *index_path)
{
	...
	if (!state->quiet) {
		...
		add_pending_oid(the_repository, &rev_info, "HEAD", &our_tree, 0);
		diff_setup_done(&rev_info.diffopt);
		run_diff_index(&rev_info, 1);
	}
...
static int clean_index(const struct object_id *head, const struct object_id *remote)
{
	...
	head_tree = parse_tree_indirect(the_repository, head);
	if (!head_tree)
		return error(_("Could not parse object '%s'."), oid_to_hex(head));

	remote_tree = parse_tree_indirect(the_repository, remote);
	if (!remote_tree)
	...
	index_tree = parse_tree_indirect(the_repository, &index);
	if (!index_tree)
