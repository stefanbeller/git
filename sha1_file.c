...
#define freshen_loose_object(r, s) freshen_loose_object_##r(s)
static int freshen_loose_object_the_repository(const unsigned char *sha1)
{
...
#define index_mem(r, s, b, sz, t, p, f) index_mem_##r(s, b, sz, t, p, f)
static int index_mem_the_repository(unsigned char *sha1,
...
#define index_stream_convert_blob(r, s, fd, p, f) \
		index_stream_convert_blob_##r(s, fd, p, f)
static int index_stream_convert_blob_the_repository(unsigned char *sha1,
				     ...
				     unsigned flags)
{
	...
	if (strbuf_read(&sbuf, fd, 4096) >= 0)
		ret = index_mem(the_repository, sha1,
...
#define index_core(r, s, fd, sz, t, p, f) index_core_##r(s, fd, sz, t, p, f)
static int index_core_the_repository(unsigned char *sha1, int fd, size_t size,
		      enum object_type type, const char *path,
		      unsigned flags)
{
	int ret;
	     ...
	     enum object_type type, const char *path, unsigned flags)
{
	...
	if (type == OBJ_BLOB && path && would_convert_to_git_filter_fd(path))
		ret = index_stream_convert_blob(the_repository, oid->hash, fd,
						path, flags);
